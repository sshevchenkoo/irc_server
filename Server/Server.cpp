...
bool Server::handleRead(int fd)
{
    ...
    if (n > 0)
    {
        Client.appendInBuf(buff, static_cast<std::size_t>(n));
...
void Server::tick(std::vector<int> &toDrop)
{
    ...
    for (std::map<int, Client>::iterator it = _clients.begin(); it != _clients.end(); ++it)
    {
        ...
        if (!Client.isAvatingPong() && now - Client.lastActive() > 240)
        {
            Client.SetPong(true);
            Client.addToOutBuf("PING :tick\r\n");
            setEvents(Client.getFd(), POLLIN | POLLOUT);
        }
        else if (Client.isAvatingPong() && now - Client.lastActive() > 250)
...
{
    client.tryRegistred();
    if (client.isRegistred() && !client.isWelcomed())
    {
